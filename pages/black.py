...
st.set_page_config(page_title="Black (1976)", layout="wide", page_icon="quantrf_logo_website.png")
...
if program_mode == "Black (1976) Pricer":
    ...
    futures_price = st.sidebar.number_input('Futures Price($)', value=100.00, format="%.2f")
    ...
    delta_call, delta_put, gamma, vega, theta_call, theta_put, rho_call, rho_put = f.calculate_black_greeks(
        futures_price, strike_price, risk_free_rate, time_to_maturity, volatility, dividend_yield=dividend_yield
    )
    ...
    futures_min = st.sidebar.number_input('Min Future Price($)', value=0.5 * futures_price, format="%.2f")
    futures_max = st.sidebar.number_input('Max Future Price($)', value=1.5 * futures_price, format="%.2f")
    ...
    with colA:
        st.markdown(f"**Future Price:** ${futures_price:.2f}")
    with colB:
    ...
    param_to_visualize = st.selectbox(
        ...
        ["Futures Price", "Strike Price", "Time to Expiration", "Interest Rate", "Volatility", "Dividend Yield"]
    )

    # ------ PARAMETER RANGES ------
    if param_to_visualize == "Futures Price":
        param_range = np.linspace(50, 150, 100)
        x_label = "Futures Price ($)"
    ...
    for param_value in param_range:
        ...
        delta_call_val, delta_put_val, gamma_val, vega_val, theta_call_val, theta_put_val, rho_call_val, rho_put_val = f.calculate_black_greeks(
            F, K, r, T, sigma, dividend_yield=q
        )

        call_price_val = f.call_black_value(F, K, r, T, sigma, q=q)
        put_price_val = f.put_black_value(F, K, r, T, sigma, q=q)
    ...
    st.header("Heatmap of Futures Prices and P&L")

    # Call and Put prices for given inputs
    call_price = f.call_black_value(futures_price, strike_price, risk_free_rate, time_to_maturity, volatility,
                                 q=dividend_yield)
    put_price = f.put_black_value(futures_price, strike_price, risk_free_rate, time_to_maturity, volatility,
                               q=dividend_yield)
    ...
    # TODO!: Switch to using the Black76 pricer function instead of the BSM pricer function for calculating option prices and Greeks.
    call_df, put_df, call_pnl_df, put_pnl_df = f.calculate_option_black_values(futures_min, futures_max, vol_min_selected,
    ...
    # User Inputs
    ticker_symbol = st.sidebar.text_input('Ticker Symbol', value='ES=F').strip().upper()
    risk_free_rate = st.sidebar.number_input('Risk-Free Rate', min_value=0.0, max_value=1.0, value=0.03, format="%.4f")
    dividend_yield = st.sidebar.number_input('Dividend Yield', min_value=0.0, max_value=1.0, value=0.0, format="%.4f")
    ...
    calls_all, puts_all, futures_price = f.get_option_chains_spot(ticker_symbol=ticker_symbol)
    ...
    min_futures, max_futures = futures_price_slider = st.sidebar.slider(
        'Futures Price Range',
        min_value=0.1 * futures_price,
        max_value=2.0 * futures_price,
        value=(0.5 * futures_price, 1.5 * futures_price),
        format="%.2f"
    )
    ...
    with colA:
        st.markdown(f"**Futures Price:** ${futures_price:.2f}")
    with colB:
    ...
    with colE:
        st.markdown(f"**Dividend Yield:** {dividend_yield:.4f}")
    with colF:
        st.markdown(f"**Futures Price Range:** ${futures_price_slider[0]:.2f} - ${futures_price_slider[1]:.2f}")
    ...
    if not all_strikes.empty:
        nearest_strike = float(all_strikes.iloc[(np.abs(all_strikes - futures_price)).argmin()])
        ...
        strike_price_hist = futures_price
        volatility_hist = 0.20
    ...
    delta_call, delta_put, gamma, vega, theta_call, theta_put, rho_call, rho_put = f.calculate_black_greeks(
        futures_price, strike_price_hist, risk_free_rate, time_to_maturity, volatility_hist, dividend_yield=dividend_yield
    )
    ...
    call_df, put_df = f.calculate_market_black_prices(min_futures, max_futures, call_datapoints, put_datapoints, risk_free_rate,
                                                                                dividend_yield)
   ...
   # --- Data (cached; only refetched when the ticker changes) ---------------
    try:
        futures_prices, futures_price = f.get_stock_data(ticker_symbol)
    ...
    prepared = f.prepare_options(options_data, futures_price)
    if prepared.empty:
    ...
    imp_vol_all = f.calculate_implied_volatility(
        prepared, futures_price, risk_free_rate, dividend_yield
    )
    ...
    min_strike_price = futures_price * (strike_pct[0] / 100)
    max_strike_price = futures_price * (strike_pct[1] / 100)
    ...
    st.caption(
        f"**{ticker_symbol}**  ·  spot ${futures_price:,.2f}  ·  {len(imp_vol_data):,} IV points "
    ...
    with surface_tab:
        if method == "SVI fit":
            fits = cached_svi_fits(imp_vol_all, futures_price, risk_free_rate, dividend_yield)
        ...
        if method == "Raw (interpolated)":
            ...
            if y_axis == "Moneyness":
                ...
                F = np.maximum(futures_price * np.exp((risk_free_rate - dividend_yield) * T), 1e-12)
    ...
    with smile_tab:
        ...
        if not expiries:
            ...
            F = futures_price * np.exp((risk_free_rate - dividend_yield) * T)
